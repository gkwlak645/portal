...
<!--  템플릿 바디 -->        
 <div class="card card-primary" id="unmanagtErrorRegDiv">                   
    <div class="card-header" alt="과제 >과제개발>프로세스상세">
        <h3 class="card-title">프로세스 임시저장</h3>
    </div>
    <div class="card-body">
        <div class="row">
            <div class="col-md-12">
                <form class="form-inline" id="unmanagtErrorDetail">
                    <input type="hidden" id="processSttus" name="processSttus" value="" />
...
<c:if test="${identityFlag}">
                    <button type="button" class="btn btn-primary" id="temporaryBtn" style="display:none">임시저장</button>
                    <button type="button" class="btn btn-primary" id="saveBtn" style="display:none">결재요청</button>
</c:if>
...
<!-- 프로세스 결재요청 창 호출  -->
<!-- <form id="PassFormDraft" name="PassFormDraft" method="post" target="_blank" action="https://gwdev.dongwon.com/WebSite/Approval/RpaDraft.aspx"> -->
<form id="PassFormDraft" name="PassFormDraft" method="post" target="_blank">
...
function bindEvent() {
    ...
    $("#temporaryBtn, #saveBtn").on("click", function() {
    	var oThis = $(this)
...
function drawChangeHistList(aList){
    ...
    for(var i = 0; i < iaList; i++){
        ...
        str += '    <td style="text-align:left">' + oaList.changeCn.replace(/(\n|\r\n)/g, '<br>'); + '</td>';
        str += '</tr>';
...
function popProcessTemporaryDetailDrawData(data){
	//데이터 화면에 출력

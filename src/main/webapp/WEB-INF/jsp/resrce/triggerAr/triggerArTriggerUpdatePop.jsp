...
<!--  템플릿 바디 -->        
 <div class="card card-primary" id="unmanagtErrorRegDiv">                   
    <div class="card-header">
        <h3 class="card-title">정기실행 수정</h3>
    </div>
    <div class="card-body">
...
function getDetailData(){
	// 날짜 세팅
	var startDate = getYYYYMMDD(old_triggerAr_info.startDe);
	var endDate = getYYYYMMDD(old_triggerAr_info.endDe);	
	$('#startDe').val(startDate);
    $('#endDe').val(endDate);

    $('#releasesSearchKwrd').val(old_triggerAr_info.processCd);
    
    // 스케줄 타입(대분류 : 매분, 매시간, 매일, 매주, 매월, 크론표현식)
    var schduleType1 = old_triggerAr_info.schduleType1;
    
    // 라디오 버튼 체크(매분, 매시간, 매일 등등)
    $('input:radio[name=schduleType1]:input[value=' + schduleType1 + ']').attr("checked", true);
    
    if(schduleType1 == 1) {
    	// 분
    	$('#everyMinute').val(old_triggerAr_info.everyMinute);
    }
	else if(schduleType1 == 2) {
		// 시간
	    $('#everyHour').val(old_triggerAr_info.everyHour);
	    $('#everyHourMinute').val(old_triggerAr_info.executMinute);
	}
	else if(schduleType1 == 3) {
		// 매일
	    $('#everyDayHour').val(old_triggerAr_info.executHour);
	    $('#everyDayMinute').val(old_triggerAr_info.executMinute);
	}
	else if(schduleType1 == 4) {
	    // 매주
	    
	    // 월~일 체크 박스 처리
	    if(old_triggerAr_info.everyWeekMonAt == "Y") {
	    	$("#everyWeekMonAt").attr("checked", true);
	    }
	    if(old_triggerAr_info.everyWeekTueAt == "Y") {
	        $("#everyWeekTueAt").attr("checked", true);
	    }
	    if(old_triggerAr_info.everyWeekWedAt == "Y") {
	    	$("#everyWeekWedAt").attr("checked", true);
        }
	    if(old_triggerAr_info.everyWeekThuAt == "Y") {
	    	$("#everyWeekThuAt").attr("checked", true);
        }
	    if(old_triggerAr_info.everyWeekFriAt == "Y") {
	    	$("#everyWeekFriAt").attr("checked", true);
        }
	    if(old_triggerAr_info.everyWeekSatAt == "Y") {
	    	$("#everyWeekSatAt").attr("checked", true);
        }
	    if(old_triggerAr_info.everyWeekSunAt == "Y") {
	    	$("#everyWeekSunAt").attr("checked", true);
        }
	    
	    $('#everyWeekHour').val(old_triggerAr_info.executHour);
	    $('#everyWeekMinute').val(old_triggerAr_info.executMinute);
	}
    else if(schduleType1 == 5) {
        // 매월
        var schduleType2 = old_triggerAr_info.schduleType2;
        $('input:radio[name=schduleType2]:input[value=' + schduleType2 + ']').attr("checked", true);
        
        if(schduleType2 == 1) {
        	// 일
        	$('#everyMonthDay').val(old_triggerAr_info.executDay);
        	$('#everyMonthHour').val(old_triggerAr_info.executHour);
            $('#everyMonthMinute').val(old_triggerAr_info.executMinute);
        }
        else if(schduleType2 == 2) {
        	// 주차
        	
        	// 몇번째 주인지
            $('input:radio[name=everyMonthWeekOrder]:input[value=' + old_triggerAr_info.everyMonthWeekOrder + ']').attr("checked", true);
        	// 요일 선택
            $('input:radio[name=everyMonthDow]:input[value=' + old_triggerAr_info.everyMonthDow + ']').attr("checked", true);
            $('#everyMonthHour2').val(old_triggerAr_info.executHour);
            $('#everyMonthMinute2').val(old_triggerAr_info.executMinute);
        }
    }   
    else if(schduleType1 == 6) {
        // 크론표현식
        $('#cronExp').val(old_triggerAr_info.schduleCrontab);  	
    }
	else {
		alert("스케줄 타입을 지정하지 않았습니다.");
		return false;
	}   
    
    $('#btnReleasesSearch').trigger('click');
        
    setTimeout(function() {
        $('#targetReleases').val(old_triggerAr_info.processCd);
        
        $("#targetReleases").trigger('change');     
        /* setTimeout(function() {
        	$('#botNo').val(old_triggerAr_info.botNo); 
            endLoading();       
        }, 500); */ 
...
function updateRoutinePlanCallback(data){
	alert("수정되었습니다.");
    $('.dimmed').css({ display: 'none' });
	$('.loading').css({ display: 'none' });
	$('.loading').removeClass('show'); 
...
*/
		...
		var param = {
                processCd: old_triggerAr_info.processCd
                , schduleNo: old_triggerAr_info.schduleNo
                , botNo: old_triggerAr_info.botNo
            }; 

        
        $.ajax({
            type: "post",
            url: "/resrce/triggerAr/deleteRoutinePlan",
            async: false,
            dataType: "json",
            	...
            	if (data.responseCode != null && data.responseCode != undefined && data.responseCode.substr(0, 1) == "E") {
                    ...
                    if(schduleType1 == "6") {
                        ...
                        if(isValidExpression) {
                            callAsyncAjax("/resrce/triggerAr/saveRoutinePlan", oParams, "updateRoutinePlanCallback");  

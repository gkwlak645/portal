            ...
            <div class="card">
                <div class="card-body">
                    <div class="cus_table-wrap">
                        ...
                        <table class="cus_table" id="robotStsList">
                            <colgroup>
...
function searchList() {
    ...
    // year 선택 조회
    if(radioVal=="Y"){
        var url = '/stats/robotExecutStatus/robotExecutStatusYearList';
        var param = {searchYear: $("#searchYear").val(), searchCmpnyCd:$("#searchCmpnyCd").val()}; 
        callAsyncAjax(url, param, "yearDrawData");
    }else{
        // month 선택 조회
        var url = '/stats/robotExecutStatus/robotExecutStatusMonthList';
 ...
 for(var j = 0; j < aResult.length ; j++){
    ...
    if(rowNo == 0 ){
        if(aResult[j].compCd != "ZZ"){
             ...
             str += '<th rowspan="3">'+aResult[j].botNm+'</th>';
        }else{
             str += '<th rowspan="3" colspan="2">전체 합계/평균</th>';
        }
     }
     if(aResult[j].gbn == "A"){

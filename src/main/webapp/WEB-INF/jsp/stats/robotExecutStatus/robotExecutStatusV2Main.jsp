...
function searchList() {
    ...
    // year 선택 조회
    if(radioVal=="Y"){
        var url = '/stats/robotExecutStatus/robotExecutStatusYearListV2';
        var param = {searchYear: $("#searchYear").val(), searchCmpnyCd:$("#searchCmpnyCd").val()}; 
        callAsyncAjax(url, param, "yearDrawData");
    }else{
        // month 선택 조회
        var url = '/stats/robotExecutStatus/robotExecutStatusMonthListV2';
 ...
 for(var j = 0; j < aResultLen; j++){
    ...
    if(rowNo == 0 ){
        if(aResult[j].compCd != "ZZ"){
             ...
             str += '<th rowspan="8">'+aResult[j].botNm+'</th>';
        }else{
             str += '<th rowspan="8" colspan="2">전체 합계/평균</th>';
        }
     }
...
function outExport(){
    ...
    var url = '/stats/robotExecutStatus/outExportrobotExecutStatusList';

...
<!-- SEARCH FORM -->
<div id="faqBoardMain" class="container-fluid wrap">
	<div class="card">
	            ...
	            <div class="row" style="padding-top: 40px;">
		            <div class="col-sm-7">
		                <div class="form-group row">
		                    ...
		                    <div class="col-sm-6">
		                        <input type="text" class="form-control" id="inputFaqSearch" placeholder="검색어">
		                    </div>
		                </div>
		            </div>
		            <div class="form-group row">
...
<script>
    ...
    function bindEvent(){
       ...
       /*  엔터 키 누를시 검색 및 그리드 작성 */
        $("#inputFaqSearch").keyup(function(e){
            if(e.keyCode == 13){
                $(".pagingClass").remove();
                getFaqList(); 
            }
        ...
        $("#regBtn").on("click", function(){
            var url = "/cmmnty/faq/faqRegPop";
            var divId = "faqBoardMain";
        ...
        $("#startDe").on("propertychange change paste input", function(){
        	if(unhyphenationPhoneNum($("#startDe").val()) > unhyphenationPhoneNum($("#endDe").val())){
                ...
                return $("#startDe").val(getMonthGapDate(-6));
                }
        });
    ...
    /*FAQ 게시글 리스트 가져오기*/
    function getFaqList(cudPage){
        var url = "/cmmnty/faq/getFaq";
        var searchCombo = $("#searchCombo").val();
        var faqSearch = $("#inputFaqSearch").val();
        ...
        if (searchCombo == "subject"){
        	param.boardSj = faqSearch;
        } else if (searchCombo == "content"){
        	param.boardCn = faqSearch;
        }
    ...
    /*FAQ 검색 조건 없는 게시글 리스트*/
    function getFaqList2(pageNum){
        var url = "/cmmnty/faq/getFaq";
        var param = new Object();
    ...
    function drawGrid(data){
        var pageSize = 10;
        if(data != ""){
        	faqList = data.result.faqList;
        	if(searchTerm == "Y"){
        		drawPager("paging", faqList.pageInfo, "getFaqList");
        	}else{
        		drawPager("paging", faqList.pageInfo, "getFaqList2");
        	}
        	
        }
        $("#faqSearchGrid").jsGrid({
            height : "500",
            width : "100%",
            autoload: true,
            sorting : true,
            rowClick: function(arg){
                ...
                var boardFaqSn = arg.item.boardSn;
                var valBoardNo = boardNo;
                
                detailFaqPageMove(boardRegId, valBoardNo);
                faqDetail(valBoardNo, boardFaqSn);
            },
            data: faqList.items,
    ...
    /*FAQ 게시글 상세페이지로 이동*/
    function detailFaqPageMove(registerId, valBoardNo){
    	var detailMoveUrl = "/cmmnty/faq/faqDetailPop";
    	var divId = "faqBoardMain";
    	var params = new Object();
    	params.registerId = registerId;
    	params.boardNo = valBoardNo;
        appendFaqDetailPopup(divId, detailMoveUrl, params);
    }
    
    /*FAQ 게시글 상세 정보*/
    function faqDetail(valBoardNo, boardFaqSn){
    var detailFaqUrl = "/cmmnty/faq/getFaqDetail";
    var param = {    "boardNo"    :  valBoardNo
                   , "boardSn"    :  boardFaqSn
                   };
    callAsyncAjax(detailFaqUrl, param, "detailFaq");
    }
    
    /*FAQ 게시글 상세페이지 그리기*/
    function appendFaqDetailPopup(divId, url, params){
        var html = "";
